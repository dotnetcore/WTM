...
{
    public static class WtmCoreServiceCollectionExtensions
    {
        public static IServiceCollection AddWtmCore(this IServiceCollection services,
        ...
        )
        {
            ...
            services.TryAdd(ServiceDescriptor.Transient<IPolicyEvaluator, WalkingTec.Mvvm.Core.Auth.PolicyEvaluator>());
            services.TryAddEnumerable(
                ServiceDescriptor.Transient<IApplicationModelProvider, WalkingTec.Mvvm.Core.Auth.AuthorizationApplicationModelProvider>());
            // edit end
            ...
            .AddDataAnnotationsLocalization(options =>
            {
                ...
                {
                    if (WalkingTec.Mvvm.Core.Program.Buildindll.Any(x => type.FullName.StartsWith(x)))
                    {
            ...
            var cookieOptions = config.GetSection("CookieOptions").Get<WalkingTec.Mvvm.Core.Auth.CookieOptions>();
            if (cookieOptions == null)
            {
                cookieOptions = new WalkingTec.Mvvm.Core.Auth.CookieOptions();
            }

            services.Configure<WalkingTec.Mvvm.Core.Auth.CookieOptions>(config.GetSection("CookieOptions"));
