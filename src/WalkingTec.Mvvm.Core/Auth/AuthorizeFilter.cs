        ...
        public virtual async Task OnAuthorizationAsync(AuthorizationFilterContext context)
        {
            ...
            if (authorizeResult.Challenged)
            {
                ...
                if (context.HttpContext.Request.Headers.ContainsKey("Authorization")
                    && effectivePolicy.AuthenticationSchemes.Contains(JwtBearerDefaults.AuthenticationScheme))
                {
                    context.Result = new ChallengeResult(JwtBearerDefaults.AuthenticationScheme);
                }
                else if (context.HttpContext.Request.Cookies.ContainsKey(CookieAuthenticationDefaults.CookiePrefix + AuthConstants.CookieAuthName)
                    && effectivePolicy.AuthenticationSchemes.Contains(CookieAuthenticationDefaults.AuthenticationScheme))
                {
