...
namespace WalkingTec.Mvvm.Mvc.Admin.ViewModels.FrameworkUserVms
{
        ...
        public override IOrderedQueryable<FrameworkUserBase_ListView> GetSearchQuery()
        {
            ...
            var query = DC.Set<FrameworkUserBase>().Include(x=>x.UserRoles).ThenInclude(r=>r.Role)
                ...
                .Select(x => new FrameworkUserBase_ListView
                {
                    ...
                    ITCode = x.ITCode,
                    Name = x.Name,
                    RoleName_view = DC.Set<FrameworkRole>().Where(y => x.UserRoles.Select(z => z.RoleId).Contains(y.ID)).Select(y => y.RoleName).ToSepratedString(null, ","),
                    GroupName_view = DC.Set<FrameworkGroup>().Where(y => x.UserGroups.Select(z => z.GroupId).Contains(y.ID)).Select(y => y.GroupName).ToSepratedString(null, ","),
                })
                .OrderBy(x => x.ITCode);
            return query;
        }
    ...
    public class FrameworkUserBaseSearcher : BaseSearcher
    {
        ...
        {
            AllRoles = DC.Set<FrameworkRole>().GetSelectListItems(WtmContext, null, x => x.RoleName, x => x.ID.ToString(), true, true);
        }
    }
