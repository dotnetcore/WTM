...
const tableData = ref({
	...
	header: [
		...
		{ key: 'IsStudent', colWidth:'', title:'是否', type:'combobox',isCheck:true,comboData:{true:'是',false:'否'}},
...
onMounted(() => {
	...
	schoolApi().get(state.vmModel.Entity.ID ?? "").then((data: any) => {
...
const onSubmit = () => {
	...
	formRef.value?.validate((valid: boolean, fields: any) => {
		if (valid) {
			schoolApi().edit(state.vmModel).then(() => {
