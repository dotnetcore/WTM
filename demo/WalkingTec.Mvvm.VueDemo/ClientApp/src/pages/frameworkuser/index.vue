<template>
  <card class="dataprivilege">
    <template v-if="isMainHost">
      <div>{{ $t("route.isMainHost") }}</div>
    </template>
    <template v-else>
      <wtm-search-box
        :ref="searchRefName"
      ...
      >
        <template #PhotoId="rowData">
          <el-image
            v-if="!!rowData.row.PhotoId"
            style="width: 100px; height: 100px"
            :src="'/api/_file/downloadFile/' + rowData.row.PhotoId"
            fit="cover"
          />
        </template>
        <template #IsValid="rowData">
          <el-switch
            :value="
              rowData.row.IsValid === 'true' || rowData.row.IsValid === true
            "
            disabled
          />
        </template>
