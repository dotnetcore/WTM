...
namespace WalkingTec.Mvvm.Mvc.Admin.ViewModels.FrameworkMenuVMs
{
    public class PagePrivilegeVM : BaseCRUDVM<FrameworkMenu>
    {
        ...
        {
            AllRoles = DC.Set<FrameworkRole>().GetSelectListItems(WtmContext, null,y => y.RoleName);
            ...
            AllUsers = DC.Set<FrameworkUserBase>().Where(x => SelectedUsersID.Contains(x.ID)).GetSelectListItems(WtmContext, null, x => x.ITCode);
            ...
            AllUsers2 = DC.Set<FrameworkUserBase>().Where(x => SelectedUsersID2.Contains(x.ID)).GetSelectListItems(WtmContext, null, x => x.ITCode);

        }

        public List<ComboSelectListItem> GetUsers(string text)
        {
            var d = DC.Set<FrameworkUserBase>().Where(x => x.ITCode.ToLower().Contains(text.ToLower())).GetSelectListItems(WtmContext, null, x => x.ITCode);
            return d;
        }
