...
namespace WalkingTec.Mvvm.Mvc.Admin.ViewModels.FrameworkMenuVMs
{
    public class PagePrivilegeVM : BaseCRUDVM<FrameworkMenu>
    {
        ...
        protected override void InitVM()
        {
            ...
            AllUsers = DC.Set<FrameworkUserBase>().Where(x => SelectedUsersID.Contains(x.ID)).GetSelectListItems(Wtm, null, x => x.ITCode);
            ...
            AllUsers2 = DC.Set<FrameworkUserBase>().Where(x => SelectedUsersID2.Contains(x.ID)).GetSelectListItems(Wtm, null, x => x.ITCode);
