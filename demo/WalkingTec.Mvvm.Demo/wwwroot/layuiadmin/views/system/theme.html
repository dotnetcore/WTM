<script type="text/html" template lay-done="layui.data.theme();">{{# 
  ...
  </div></script><script>layui.data.theme=function(){layui.use("form",function(){var i=layui.form,e=layui.admin;i.on("switch(system-theme-sideicon)",function(){e.theme({hideSideIcon:this.checked})})})}</script>