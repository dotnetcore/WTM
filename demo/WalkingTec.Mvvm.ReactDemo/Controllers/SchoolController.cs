...
namespace WalkingTec.Mvvm.ReactDemo.Controllers
{
	...
	public partial class SchoolController : BaseApiController
    {
        ...
        {
            return Ok(DC.Set<City>().GetSelectListItems(WtmContext, null, x => x.Name));
        }

        [HttpGet("GetSubCities")]
        public ActionResult GetSubCities(Guid? parentid)
        {
            var city = DC.Set<City>().Where(x => x.ParentId == parentid).GetSelectListItems(WtmContext, null, x => x.Name);
            return Ok(city);
        }
